...
/* HACK HACK HACK */
#define LWLockAcquire(x,y)
#define LWLockRelease(x)
#define LWLockConditionalAcquire(x,y) 1
#undef START_CRIT_SECTION
#define START_CRIT_SECTION()
#undef END_CRIT_SECTION
#define END_CRIT_SECTION()
#define pg_fsync fsync
#define OpenTransientFile(fileName, fileFlags, fileMode) open(fileName, fileFlags, fileMode)
#define CloseTransientFile(fd) close(fd)
#define AllocateDir(dirname) opendir(dirname)
#define FreeDir(dir) closedir(dir)
#define ReadDir(dir, dirname) readdir(dir)
#define XLogFlush(x)
#define InRecovery 1
#define IsUnderPostmaster 0
			  ...
			  LWLockId ctllock, const char *subdir)
{
	...
	shared = (SlruShared)malloc(SimpleLruShmemSize(nslots, nlsns));
	...
	if (!IsUnderPostmaster)
	{
		...
		for (slotno = 0; slotno < nslots; slotno++)
		{
			...
			shared->buffer_locks[slotno] = (LWLockId)0; /* HACK HACK HACK */
			ptr += BLCKSZ;
		}
	}
...
SlruPhysicalReadPage(SlruCtl ctl, int pageno, int slotno)
{
	...
	if (fd < 0)
	{
		...
		fprintf(stderr, "LOG: file \"%s\" doesn't exist, reading as zeroes", path);
		MemSet(shared->page_buffer[slotno], 0, BLCKSZ);
		return true;
	}
...
SlruReportIOError(SlruCtl ctl, int pageno, TransactionId xid)
{
	...
	switch (slru_errcause)
	{
		case SLRU_OPEN_FAILED:
			fprintf(stderr, "could not access status of transaction %u: Could not open file \"%s\": %m.", xid, path);
			break;
		case SLRU_SEEK_FAILED:
			fprintf(stderr, "could not access status of transaction %u", xid);
			fprintf(stderr, "Could not seek in file \"%s\" to offset %u: %m.",
						   path, offset);
			break;
		case SLRU_READ_FAILED:
			fprintf(stderr, "could not access status of transaction %u", xid);
			fprintf(stderr,"Could not read from file \"%s\" at offset %u: %m.",
						 path, offset);
			break;
		case SLRU_WRITE_FAILED:
			fprintf(stderr, "could not access status of transaction %u", xid);
			fprintf(stderr,"Could not write to file \"%s\" at offset %u: %m.", path, offset);
			break;
		case SLRU_FSYNC_FAILED:
			fprintf(stderr, "could not access status of transaction %u", xid);
			fprintf(stderr, "Could not fsync file \"%s\": %m.", path);
			break;
		case SLRU_CLOSE_FAILED:
			fprintf(stderr, "could not access status of transaction %u", xid);
			fprintf(stderr, "Could not close file \"%s\": %m.", path);
			break;
		default:
			/* can't get here, we trust */
			fprintf(stderr, "ERROR: unrecognized SimpleLru error cause: %d",
				 (int) slru_errcause);
			break;
	}
}
...
restart:;
	...
	{
		LWLockRelease(shared->ControlLock);
		fprintf(stderr, "LOG: could not truncate directory \"%s\": apparent wraparound",
				  ctl->Dir);
		return;
	}
